variable "pvs_data_center" {
  description = "Where to deploy the Power VS Workspace"
  default = "wdc07"
}

variable "workspace_a_name" {
  description = "The name of the workspace to create"
  default = "techxchange-pvs-workspace"
}

variable "workspace_a_subnet_name" {
  description = "This wil be the name of the subnet in workspace a"
  default = "main"
}

variable "workspace_a_subnet" {
  description = "This is the CIDR for the subnet that will be created in workspace a"
  default = "192.168.0.0/24"
}

variable "ssh_public_key" {
  description = "This is the public SSH key to use in IBM Cloud"
  default = "ssh-rsa "
}

variable "ssh_key_name" {
  description = "This will be the name used for the public ssh key"
  default = "techxchange-ssh-key"
}
